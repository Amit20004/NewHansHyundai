...
export default function CarOffersAdmin() {
  ...
  const fetchCarOffers = async () => {
    try {
      const res = await axios.get("http://localhost:8000/api/car-offers")
      if (res.data.success) {
        setCarOffers(res.data.data)
      }
    } catch (err) {
      console.error("Error fetching car offers:", err)
      toast.error("Failed to fetch car offers")
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        response = await axios.put(`http://localhost:8000/api/car-offers/${editingId}`, formDataToSend, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
        toast.success("Car offer updated successfully")
      } else {
        // Add new record
        response = await axios.post("http://localhost:8000/api/car-offers", formDataToSend, {
          headers: {
            "Content-Type": "multipart/form-data"
          }
        })
        toast.success("Car offer added successfully")
      }
      
      fetchCarOffers()
      resetForm()
    } catch (err) {
      console.error("Error saving car offer:", err)
      if (err.response && err.response.data) {
        toast.error(`Error: ${err.response.data.message || "Failed to save car offer"}`)
      } else {
        toast.error("Failed to save car offer. Please check your connection.")
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleEdit = (carOffer) => {
    // Find the specific variant if needed, or use the first one
    const variant = carOffer.variants[0]
    
    setEditingId(variant.id)
    setFormData({
      car_name: carOffer.car_name || "",
    ...
    if (variant.images.main) {
      setCardPreview(variant.images.main)
    } else {
      setCardPreview(null)
    }
    
    if (variant.images.thumbnail) {
      setThumbnailPreview(variant.images.thumbnail)
    } else {
      setThumbnailPreview(null)
    }
    
    // Clear file states as we'll need new files if updating
    setCardImage(null)
    setThumbnailImage(null)
  ...
  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this car offer?")) return
    
    try {
      const res = await axios.delete(`http://localhost:8000/api/car-offers/${id}`)
      if (res.data.success) {
        fetchCarOffers()
        toast.success("Car offer deleted successfully")
      }
    } catch (err) {
      console.error("Error deleting car offer:", err)
      toast.error("Failed to delete car offer")
    }
  }
  ...
  const filteredOffers = carOffers
    .filter((offer) => {
      const matchesSearch =
    ...
    .sort((a, b) => {
      // Since we don't have dates in the response, we'll sort by car name
      switch (sortBy) {
        case "name_asc":
      ...
      {showForm && (
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6">
          <h2 className="text-lg font-semibold mb-4">
            {editingId ? "Edit Car Offer" : "Add New Car Offer"}
          </h2>
              ...
              <div className="flex flex-col items-start gap-2">
                ...
                {cardPreview && (
                  <div className="relative">
                    <Image
                    width={100} height={100} quality={100} unoptimized={true}s
                      src={cardPreview} 
                      alt="Card Preview" 
